...
OutputAction LeftMidLowSoundOutput = {"tPWM1", 4};  // tPWM1 left sound 
OutputAction LeftMidHighSoundOutput = {"tPWM1", 5};  // tPWM1 left sound 

OutputAction LeftSoundOutput[6] = {DummyOutput ,LeftLowSoundOutput ,LeftMidLowSoundOutput ,LeftCueSoundOutput ,LeftMidHighSoundOutput ,LeftHighSoundOutput};
...
OutputAction RightMidLowSoundOutput = {"tPWM3", 4}; // tPWM3 right sound 
OutputAction RightMidHighSoundOutput = {"tPWM3", 5}; // tPWM3 right sound 

OutputAction RightSoundOutput[6] = {DummyOutput ,RightLowSoundOutput ,RightMidLowSoundOutput ,RightCueSoundOutput ,RightMidHighSoundOutput ,RightHighSoundOutput};
...
void construct_matrix_and_Run()
{
  ...
  {            
    ...
    StateTransition PreCueDelayPeriod_Cond[3] = {{"Tup", ActionAfterDelay} ,{"Lick1In", "EarlyLickAborted"}, {"Lick2In", "EarlyLickAborted"}};
  ...
  case 5: // Test 1 :   sample left 
  case 6: // Test 2: sample right rule reversal
  case 7: // Test 3 :   trial blocks training 50 trials 
  case 8://  Test 4:  random 
  {        
    S.PreCueDelayPeriod = float(random(200, 500));
    ...
    {
    case 2:
      modality = 0; // left ori or right ori
      conflict = 0; // 
      TrialBlock = 0;
      // timeout & ITI
    ...
    case 3:
    case 4:
      modality = 0; // rule 2 ,1 reverasl or not ; 3 probe trials : top sample orientation 10% ; 0 not used
      conflict = 0; // not used
      TrialBlock = 0; // not used
      break;
    
    case 5: // test subprotocol
      SampleOutput = DummyOutput;
      modality = 1;  // not reversal
      break;
    case 6:
      SampleOutput = DummyOutput;
      modality = 2; // reversal
    ...
    case 7:
      SampleOutput = DummyOutput;
      if((int)(S.currProtocolTrials % 50) == 0){
        // trial block 100
        modality = int(random(1 ,3)); // reversal or not reversal per blocks
      }
      if(modality == 0){
        modality = int(random(1 ,3));
      }
      break;
    case 8:
      SampleOutput = DummyOutput;
    ...
    case 1: // left choice ; 3khz 
    {
      if(modality == 1){ // not reversal left sound
        SampleOutputL = LeftLowSoundOutput;
        SampleOutputR = DummyOutput;
      }
      else if(modality == 2){ // reversal right sound
        SampleOutputL = DummyOutput;
        SampleOutputR = RightHighSoundOutput;
      }else if(modality == 3){ // probe trials not reverasl top sound
        SampleOutput = LowSoundOutput;
        SampleOutputL = DummyOutput;
        SampleOutputR = DummyOutput;
      }else{
    ...
    {
      if(modality == 1){ // not reversal left sound
        SampleOutputL = LeftHighSoundOutput;
        SampleOutputR = DummyOutput;
      }
      else if(modality == 2){ // reversal right sound
        SampleOutputL = DummyOutput;
        SampleOutputR = RightLowSoundOutput;
      }else if(modality == 3){ // probe trials not reverasl top sound
        SampleOutput = HighSoundOutput;
        SampleOutputL = DummyOutput;
        SampleOutputR = DummyOutput;
...
void autoChangeProtocol()
{
  ...
  case 5: // Test 1 not reversal
    if (S.currProtocolTrials > 100 && int(currProtocolPerf_corrected * 100) > 75)
    {
      S.retention_counter++;
      if(S.retention_counter > 20){
        S.currProtocolIndex = 7;  // Test 3 block
      }else{
        S.currProtocolIndex = 6;  // Test 2 reversal
      }
      S.currProtocolTrials = 0;
      S.currProtocolPerf = 0;
  ...
  case 6: // Test 2 reversal training
    if (S.currProtocolTrials > 100 && int(currProtocolPerf_corrected * 100) > 75)
    {
      ...
      if(S.retention_counter > 20){
        ...
        S.currProtocolIndex = 5;  // Test 1 nor reversal
      }
      S.currProtocolTrials = 0;
      S.currProtocolPerf = 0;
  ...
  case 7: // block
    if (S.currProtocolTrials > 5000 && int(currProtocolPerf_corrected * 100) > 75)
    {
      S.currProtocolIndex = 8;  // random 
      S.currProtocolTrials = 0;
      S.currProtocolPerf = 0;
...
int write_SD_trial_info()
{
  ...
  if (dataFile)
  {
    ...
    dataFile.print(conflict); 
    dataFile.print(" ");

    dataFile.print(TrialBlock); 
    dataFile.print(" ");
