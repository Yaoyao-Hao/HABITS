...
typedef struct
{
  ...
  int extra_TimeOut = 0;
  bool light_Flag = 0; // test protocol light intensirty choice flag
...
{ // 使用 trial by trial 的更新方式 ；其中超参数有： 1. Fast EMA alpha ; 2. Slow EMA alpha ; 3. ALP EMA alpha 
  float FastEMA[5] = {0}; // EMA 500
  float SlowEMA[5] = {0}; // EMA 1000
  float ALP[5] = {0};

  int CurrParamsIndex = 0; // 0 or 1
  int subParamsNum = 5; 
  int miniBatch    = 100; // 每次选择得到的一个参数都会连续训练 n 个trials ： trialBlock
  int currBatchIndex = 0;

  float FastAlpha = 0.99; // average 500
  float SlowAlpha = 0.999; // average 1000
  float ALPAlpha  = 0.999; // average 1000

  float e_greedy = 10; // 5% percent random choice the params
  float NormSoftMax_T = 0.1; 

  unsigned int t[5] = {1 ,1 ,1 ,1 ,1};                  // update step
} CurriculumParams;
CurriculumParams CP;

int CurriculumSpace[5][3] = {{0 ,0 ,0} ,{0 ,0 ,1} ,{1 ,0 ,2} ,{2 ,0 ,0} ,{2 ,0 ,1}}; ; // ContextType 
...
OutputAction AudContrastOutput[4] = {LowContrHighSoundOutput , LowContrHighSoundOutput ,MidLowContrHighSoundOutput ,MidLowContrHighSoundOutput}; // 1 trained ；
...
int modality_LT_Low[3] = {0 ,2 ,3};
int modality_LT_High[3] = {1 ,2 ,4};

// 其中 ，modality 参数 对应 trialparam rows；然后trialtype 对应 left or right
/********** FlasherX **********/
...
void setup()
{
  ...
  smart.setTruePWMFrequency(2, 2, 10000, 32); // (high sound) byte tPWM_num, byte freq_num, uint32 frequency, byte duty
  smart.setTruePWMFrequency(2, 3, 6500, 128);  // (go cue) byte tPWM_num, byte freq_num, uint32 frequency, byte duty
  //TODO
  smart.setTruePWMFrequency(2, 4, 10000, 16); // (high sound) byte tPWM_num, byte freq_num, uint32 frequency, byte duty
  smart.setTruePWMFrequency(2, 5, 10000, 32); // (high sound) byte tPWM_num, byte freq_num, uint32 frequency, byte duty
  smart.setTruePWMFrequency(2, 6, 10000, 64); // (high sound) byte tPWM_num, byte freq_num, uint32 frequency, byte duty
//   OutputAction HighSoundOutput         = {"tPWM2", 2}; // bigest contrast
// OutputAction LowContrHighSoundOutput = {"tPWM2", 4}; // lowest contrast
// OutputAction MidLowContrHighSoundOutput = {"tPWM2", 5}; // midlow
// OutputAction MidHighContrHighSoundOutput = {"tPWM2", 6}; // midhigh
...
void construct_matrix_and_Run()
{
  ...
  if(CurriculumSpace[modality][2] == 0){
  ...
  }else if(CurriculumSpace[modality][2] == 1){
  ...
  {
		int modality_choice = int(random(3));
		if(S.light_Flag){
			// low 
			modality = modality_LT_Low[modality_choice];
		}else{
			// high
			modality = modality_LT_High[modality_choice];
		}
    ...
    if (int(random(100)) < CatchTrialProb)
    {
      ...
      if(S.bright_flag == 0){
        LeftLightOutput = {"PWM1", int(LT_list[CurriculumSpace[modality][2]] / 2)};  // left light, value 0-255,TODO just 1 ,the light intensity is unaffordable(anlogWrite()command)
        RightLightOutput = {"PWM5", int(LT_list[CurriculumSpace[modality][2]] / 2)}; // right light ,same as above
      }else if(S.bright_flag == 1){
        LeftLightOutput = {"PWM1", int(LT_list[CurriculumSpace[modality][2]] * 2)};  // left light, value 0-255,TODO just 1 ,the light intensity is unaffordable(anlogWrite()command)
        RightLightOutput = {"PWM5", int(LT_list[CurriculumSpace[modality][2]] * 2)}; // right light ,same as above
      }else{ // undef
        LeftLightOutput = {"PWM1", LT_list[CurriculumSpace[modality][2]]};  // left light, value 0-255,TODO just 1 ,the light intensity is unaffordable(anlogWrite()command)
        RightLightOutput = {"PWM5", LT_list[CurriculumSpace[modality][2]]}; // right light ,same as above
      }
  ...
  {                             // PROTOCOL 0: teaching animal to lick both side
    ...
    {
      if(CurriculumSpace[modality][1] == 0){
        PoissonRate = 4;
      }else if(CurriculumSpace[modality][1] == 1){
        PoissonRate = 8;
      }
  ...
  while(EventRate != PoissonRate) // set the event rate to a constant
  {
    ...
    switch (CurriculumSpace[modality][0])
    {
    case 0:
    {                         // visual only
  ...
  {   
    ...
    // contrast aud cue
    AudContrast = int(random(1 ,5)); // 1 ~ 3 contrast ; 4 standard
...
void autoChangeProtocol()
{
  ...
  case 7:  // 关注于 light flashes protocol
	if (S.currProtocolTrials > 100 && (CP.FastEMA[0] >= 0.75 || CP.FastEMA[1] >= 0.75 ))
    { 
    if(CP.FastEMA[0] >= 0.75){
      S.light_Flag = 0;
    }else{
      S.light_Flag = 1;
    }
...
/*****************************************curriculum training --- sub traks control******************************************/
void curriculumChangeProtocol()
{
  ...
  if (TrialOutcome != 0 && TrialOutcome != 3){
    ...
    // including the EarlyLick trial
     // 第一步： 计算最新的ALP值;
    CP.FastEMA[CP.CurrParamsIndex] = CP.FastEMA[CP.CurrParamsIndex] * CP.FastAlpha + (1.0 - CP.FastAlpha) * currReward; // Fast EMA
    CP.SlowEMA[CP.CurrParamsIndex] = CP.SlowEMA[CP.CurrParamsIndex] * CP.SlowAlpha + (1.0 - CP.SlowAlpha) * currReward; // Slow EMA
    // reweight
    ...
    float FSEMA = 0;
    corrected_FastEMA = CP.FastEMA[CP.CurrParamsIndex] / (1.0 - pow(CP.FastAlpha, CP.t[CP.CurrParamsIndex]));
    corrected_SlowEMA = CP.SlowEMA[CP.CurrParamsIndex] / (1.0 - pow(CP.SlowAlpha, CP.t[CP.CurrParamsIndex]));
    
    corrected_FastEMA = sigmoid_reweight(corrected_FastEMA);
    corrected_SlowEMA = sigmoid_reweight(corrected_SlowEMA);

    FSEMA = corrected_FastEMA - corrected_SlowEMA;

    // 第二步 ： 更新 ALP Q list
    CP.ALP[CP.CurrParamsIndex] = (1.0 - CP.ALPAlpha) * abs(FSEMA) + CP.ALPAlpha * CP.ALP[CP.CurrParamsIndex];
    
    CP.t[CP.CurrParamsIndex]++;
    CP.currBatchIndex++;
  }
  
  // softmax + e-greedy choice the params
  if(CP.currBatchIndex >= CP.miniBatch){
    if(int(random(100) < CP.e_greedy)){
      CP.CurrParamsIndex = int(random(0 ,CP.subParamsNum));
    }else{
    CP.CurrParamsIndex = normSoftMaxMin(CP.subParamsNum ,CP.ALP ,CP.NormSoftMax_T ,0 ,0); // softmax without normlization
    }
    modality = CP.CurrParamsIndex;
    CP.currBatchIndex = 0;
  }
...
int write_SD_para_S()
{
  ...
  if (dataFile)
  {
	...
	// light_Flag
    dataFile.print("light_Flag = ");
    dataFile.println(S.light_Flag);
    // bright_flag
...
int normSoftMaxMin(int length , float *data ,float Gammar ,int mode ,int norm)
{
  ...
  if(norm){
    // normlize the Q value
    for(int l=0;l<length;l++){ // 遍历所有的数组元素
    temp_Q = data[l];
    // correct
    temp_Q = temp_Q / (1.0 - pow(CP.ALPAlpha, CP.t[l]));
	...
	for (int j = 0; j < length; j++)
	{
	  ...
	  TrialProb[j] = exp(mode_flag * ((data[j] / (1.0 - pow(CP.ALPAlpha, CP.t[j]))) - minQ)/ (maxQ - minQ) / Gammar);
	  sum = sum + TrialProb[j];
	}
...
int read_Curriculum_Param_info()
{
  ...
  if (dataFile)
  {
    ...
    // FastEMA
    string_tmp = dataFile.readStringUntil('=');
    for (int i = 0; i < CP.subParamsNum; i++)
    {
      string_tmp = dataFile.readStringUntil(';');
      CP.FastEMA[i] = string_tmp.toFloat();
    }
    string_tmp = dataFile.readStringUntil('\n');
    ...
    {
      string_tmp = dataFile.readStringUntil(';');
      CP.SlowEMA[i] = string_tmp.toFloat();
    }
    string_tmp = dataFile.readStringUntil('\n');
    ...
    string_tmp = dataFile.readStringUntil('=');
    string_tmp = dataFile.readStringUntil('\n');
    CP.CurrParamsIndex  = string_tmp.toInt();
    
    // subParamsNum
    string_tmp = dataFile.readStringUntil('=');
    string_tmp = dataFile.readStringUntil('\n');
    CP.subParamsNum = string_tmp.toInt();

    // miniBatch
    string_tmp = dataFile.readStringUntil('=');
    string_tmp = dataFile.readStringUntil('\n');
    CP.miniBatch = string_tmp.toInt();

    // currBatchIndex
    string_tmp = dataFile.readStringUntil('=');
    string_tmp = dataFile.readStringUntil('\n');
    CP.currBatchIndex = string_tmp.toInt();
    ...
    string_tmp = dataFile.readStringUntil('=');
    string_tmp = dataFile.readStringUntil('\n');
    CP.ALPAlpha = string_tmp.toFloat();

    // e_greedy 
    string_tmp = dataFile.readStringUntil('=');
    string_tmp = dataFile.readStringUntil('\n');
    CP.e_greedy = string_tmp.toFloat();
...
int write_Curriculum_Param_info()
{
  ...
  if (dataFile)
  {
    ...
    for (int i = 0; i < CP.subParamsNum; i++)
    {
      dataFile.print(CP.ALP[i] ,6);
      dataFile.print(";");
    }
    dataFile.println();

    // CurrParamsIndex 
    dataFile.print("CurrParamsIndex = ");
    dataFile.println(CP.CurrParamsIndex);

    // subParamsNum 
    dataFile.print("subParamsNum = ");
    dataFile.println(CP.subParamsNum);

    // miniBatch 
    dataFile.print("miniBatch = ");
    dataFile.println(CP.miniBatch);

    // currBatchIndex 
    dataFile.print("currBatchIndex = ");
    dataFile.println(CP.currBatchIndex);
    ...
    dataFile.println(CP.SlowAlpha ,5);

    // ALPAlpha 
    dataFile.print("ALPAlpha = ");
    dataFile.println(CP.ALPAlpha ,5);

    // e_greedy 
    dataFile.print("e_greedy = ");
    dataFile.println(CP.e_greedy ,5);
...
int write_Curriculum_model_info()
{
  ...
  if (dataFile)
  {
    ...
    // hyper params
    dataFile.print(" ");
    dataFile.print(CP.CurrParamsIndex);
    dataFile.print(" ");
    dataFile.print(CP.currBatchIndex);
    // ALP
    for (int i = 0; i < CP.subParamsNum; i++)
    {
      dataFile.print(" ");
...
float sigmoid_reweight(float data){
  ...
  // sigmoid reweight
  reweightValue = 1.0/(1.0 + exp(-(reweightValue*10.0 - 4.0))); 
