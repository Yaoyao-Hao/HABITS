...
void construct_matrix_and_Run()
{
  ...
  {            
    ...
    states[10] = smart.CreateState("TimeOutEL",                      random(3 ,9) * 1000,       1,                  Tup_Exit_Cond,                0,           NoOutput);
    states[12] = smart.CreateState("EndCue",                         100,                       1,                  EndCue_Cond,                  1,           EndCue_Output);
    states[2] = smart.CreateState("TrialEnd",                        random(30, 60) * 60 * 1000,2,                  TrialEnd_Cond,                0,           NoOutput);
  ...
  case 2: // Early training 1 timeout 2000 -> 8000 ; ITI 200 -> 1000 ;every step 100 trials ;total 500 trials
  case 3: // Early training 2 
  case 4:  // retention protocol rule 0

  case 5: // Test 1 :  rule 1
  case 6: // Test 2 :  rule 2
  case 7:  // rule 3
  // without  error trials
  case 8: // rule 0
  case 9: // rule 1
  case 10: // rule 3
  {        
    ...
    case 2:
      // timeout & ITI
      switch ((int)(S.currProtocolTrials / 100))
      {
        case 0:
          S.TimeOut = 2000;
          S.StopLickingPeriod = 200;
        break;
        case 1:
          S.TimeOut = 2000;
    ...
    case 1: // left choice ; 3khz 
    {
      ...
      if(NoError == 1){
        RightLickAction = "ErrorTest";
        RightLickActionSample = "ErrorTest";
      }
      RewardOutput = LeftWaterOutput;
      reward_dur = S.reward_left;
    }
    ...
    case 2: // right choice ; 12khz
    {
      ...
      if(NoError == 1){
        LeftLickAction = "ErrorTest";
        LeftLickActionSample = "ErrorTest";
      }
      RewardOutput = RightWaterOutput;
      reward_dur = S.reward_right;
    }
    ...
    states[11] = smart.CreateState("EarlyLickAborted",               500              ,         1,                  EarlyLickAborted_Cond,        1,           ErrorOutput); 
...
void UpdateTrialOutcome()
{
  ...
  */
  ...
  if (S.OutcomeHistory[RECORD_TRIALS - 1] != 3 && S.OutcomeHistory[RECORD_TRIALS - 1] != 0)
  {
    ...
    S.currProtocolPerf = S.currProtocolPerf * Alpha100 + (1.0 - Alpha100) * CurrOutcome;
  }
  currProtocolPerf_corrected = S.currProtocolPerf / (1.0 - pow(Alpha100 ,S.currProtocolTrials));
...
{ 
  ...
  case 3: // Early training 2 ; training
  ...
  case 4: // Retention period
    if (S.currProtocolTrials % 100 == 0 && int(currProtocolPerf_corrected * 100) > 75)
    {
      S.retention_counter++;
    }
    if(S.retention_counter >= 3){
  ...
  case 6: // Test 2 reversal training
    if (S.currProtocolTrials % 100 == 0 && int(currProtocolPerf_corrected * 100) > 75)
    {
      S.retention_counter++;
    }
    if(S.retention_counter >= 3){
      S.currProtocolIndex = 7;  // Test 1
      S.currProtocolTrials = 0;
      S.currProtocolPerf = 0;
      S.retention_counter = 0;
  ...
  case 7: // Test 2 reversal training
    if (S.currProtocolTrials % 100 == 0 && int(currProtocolPerf_corrected * 100) > 80 && TrialOutcome != 0 && TrialOutcome != 3)
    {
      S.retention_counter++;
    }
    if(S.retention_counter >= 3){
    ...
    break;
    // test 8 ,9 ,10 对应于 cue 1 ； cue 2 ；reversal cue2
  case 8: // Test 2 reversal training
    if (S.currProtocolTrials % 100 == 0 && int(currProtocolPerf_corrected * 100) > 80 && TrialOutcome != 0 && TrialOutcome != 3)
    {
      S.retention_counter++;
    }
    if(S.retention_counter >= 3){
      S.currProtocolIndex = 9;  // Test 1
      S.currProtocolTrials = 0;
      S.currProtocolPerf = 0;
      S.retention_counter = 0;
    ...
    break;
  case 9: // Test 2 reversal training
    if (S.currProtocolTrials % 100 == 0 && int(currProtocolPerf_corrected * 100) > 80 && TrialOutcome != 0 && TrialOutcome != 3)
    {
      S.retention_counter++;
    }
    if(S.retention_counter >= 3){
      S.currProtocolIndex = 10;  // Test 1
      S.currProtocolTrials = 0;
      S.currProtocolPerf = 0;
      S.retention_counter = 0;
    ...
    break;
  case 10: // Test 2 reversal training
    if (S.currProtocolTrials % 100 == 0 && int(currProtocolPerf_corrected * 100) > 80 && TrialOutcome != 0 && TrialOutcome != 3)
    {
      S.retention_counter++;
    }
    if(S.retention_counter >= 3){
      S.currProtocolIndex = 8;  // Test 1
      S.currProtocolTrials = 0;
      S.currProtocolPerf = 0;
      S.retention_counter = 0;
...
void trialSelection() { // 
  ...
  if(modelfitFlag == 1){
    if(S.OutcomeHistory[RECORD_TRIALS - 1] == 0 || S.OutcomeHistory[RECORD_TRIALS - 1] == 3) // outcome == 3 : earlylick  ;outcome == 0 : NoResponse trials ;
    {
      ...
      for (int i = RECORD_TRIALS - 2; i >= 0; i--){
        if(S.OutcomeHistory[i] != 3){ // remove Earlylick trials
          if(S.OutcomeHistory[i] != 0){
            ...
            // Action History
            if(S.SampleTypeHistory[i] == 1) // attend to stimu 1
            {
              A_n_Back = S1_n_Back * R_n_Back;
            }else if(S.SampleTypeHistory[i] == 2) // attend to stimu2
            {
              A_n_Back = S2_n_Back * R_n_Back;
            }
          }
        break;
        }
      ...
      for (int i = RECORD_TRIALS - 2; i >= 0; i--){
        ...
        if(S.OutcomeHistory[i] != 3){ // remove Earlylick trials
          if(S.OutcomeHistory[i] != 0){
            ...
            if(S.SampleTypeHistory[i] == 1) // attend to stimu 1
            {
              A_n_Back_avg = A_n_Back_avg * 0.9 + (1.0 - 0.9) * (S1_temp * R_temp);
            }else if(S.SampleTypeHistory[i] == 2) // attend to stimu2
            {
              A_n_Back_avg = A_n_Back_avg * 0.9 + (1.0 - 0.9) * (S2_temp * R_temp);
            }
            counter_g++;
          }
          if(counter_g >= 10){
      ...
      // WS & LS
      if(MiceChoiceData.x[6] == 1){ // win
        MiceChoiceData.x[11]    = MiceChoiceData.x[3] * MiceChoiceData.x[5] * currStimu[0];
        MiceChoiceData.x[12]    = MiceChoiceData.x[4] * MiceChoiceData.x[5] * currStimu[1];
      ...
      else if(MiceChoiceData.x[6] == -1){ // lose
        MiceChoiceData.x[11]    = 0;
        MiceChoiceData.x[12]    = 0;
        // reversal training LS
        MiceChoiceData.x[13]    = MiceChoiceData.x[3] * MiceChoiceData.x[5] * currStimu[0]; 
        MiceChoiceData.x[14]   = MiceChoiceData.x[4] * MiceChoiceData.x[5] * currStimu[1];
        // rule switch LS
        MiceChoiceData.x[15]    = MiceChoiceData.x[3] * MiceChoiceData.x[5] * currStimu[1]; 
        MiceChoiceData.x[16]   = MiceChoiceData.x[4] * MiceChoiceData.x[5] * currStimu[0];  
      }
      else{
        MiceChoiceData.x[11]    = 0;
        MiceChoiceData.x[12]    = 0;
...
void rule_define(int rule , int stimu1 ,int stimu2){
  // 给出刺激和rule : 得到y (correct answer) ,trial type ,sample type W_star
  // zero W_star
    for(int i=0;i<18;i++){
      W_star[i] = 0.0;
    }
    ...
    else if(rule == 3){ // attend to stimu2 and reversal learning {0 ,-10}
        SampleType = 2;
        W_star[2]  = -10.0;
        if(stimu2 == -1){
          TrialType = 2; 
...
float rule_search(int rule , int stimu1 ,int stimu2){
  // 给出刺激和rule : 得到y (correct answer) ,trial type ,sample type W_star
  // zero W_star
    for(int i=0;i<18;i++){
      W_star[i] = 0.0;
...
void SelectTrialType(struct DATAModel *data ,byte Kernel ,byte rule) { 
    ...
    for(int i=0;i<4;i++){ 
      ...
      data->y = rule_search(int(rule) ,int(stimu1) ,int(stimu2));
      data->x[1] = stimu1;
      data->x[2] = stimu2;
      // WS & LS
      if(data->x[6] == 1){ // win
        data->x[11]    = data->x[3] * data->x[5] * stimu1;
        data->x[12]    = data->x[4] * data->x[5] * stimu2;
      ...
      else if(data->x[6] == -1){ // lose
        data->x[11]    = 0;
        data->x[12]    = 0;
        // reversal training LS
        data->x[13]    = data->x[3] * data->x[5] * stimu1; 
        data->x[14]   = data->x[4] * data->x[5] * stimu2;
        // rule switch LS
        data->x[15]    = data->x[3] * data->x[5] * stimu2; 
        data->x[16]   = data->x[4] * data->x[5] * stimu1;  
      }
      else{
        data->x[11]    = 0;
        data->x[12]    = 0;
      ...
      for(int w=0;w<18;w++){
        delta_W[i][w] = (yhat - data->y) * data->x[w];
        delta_W[i][w] = delta_W[i][w] * float(actionmodel.Eta);
      }
      // calc the argMin value
      float T1 = 0; // difficulty item
      float T2 = 0; // usefulness item
      for(int w = 0;w<18;w++){
        T1 += pow(delta_W[i][w] ,2);
        T2 += delta_W[i][w] * delta_W_Star[w];
      }
      ArgMinValue[i] = pow(actionmodel.Gamma ,2) * T1 * float(Kernel) - 2.0 * actionmodel.Gamma * T2;
      Serial.print(ArgMinValue[i] ,4);
      Serial.print(" ");
      Serial.println();
    }  
    ...
    // output
    currStimu[0] = SGM[0][int(min[1])];
    currStimu[1] = SGM[1][int(min[1])];
}
