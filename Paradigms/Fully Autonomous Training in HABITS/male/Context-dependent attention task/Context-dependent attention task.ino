...
/********** Trial related **********/
#define RECORD_TRIALS 500      // record recent 100 trials history
...
typedef struct
{
  ...
  float currProtocolPerf[4] = {0};           // performance: 0-100% ; hardest trials
  ...
  byte ModalityHistory[RECORD_TRIALS] = {}; // 0 visual ;1 auditory ;2-undef
  byte ContextHistory[RECORD_TRIALS] = {}; 
  ...
  int extra_TimeOut = 0;
  unsigned int hardestTrials[4] = {0};
} Parameters_behavior;
...
int modality = 0; // 0: visual target ; 1: auditory target ;2 :nodef
int ContextType = 0; // 0 is without-conflict  ;1 is conflict ;2 is coherent;3:nodef
int PreCueContrast = 0; // 0-6

int CatchTrialProb = 30; // use a small number of trials as catch trials
...
float currProtocolPerf_corrected[4] = {0};
int BlockSize = 6; 

float Alpha50 = 0.98;
float Alpha100 = 0.99;

int PreCueInterval[7] = {58 ,45 ,35 ,26 ,20 ,15 ,11};

/*******************Curriculum Training based on block**************************/


byte ELInhibitaionStage[4] = {0} ; // 4 stage : 1 -> onset delay ; 2-> precue ; 3 -> Delay 1 ; 4 -> delay 2 ; value :0 or 2 
...
void loop()
{
  ...
  { // if yes, run the state matrix
    ...
    { // i.e., a trial is done
      ...
      UpdateTrialOutcome();  // including white noi se (if error) and inter-trial interval
...
void construct_matrix_and_Run()
{
  ...
  OutputAction GreenLightOutput = {"Flag3" , 0};          // Green light...
  OutputAction GreenLightOutputBackground = {"PWM3", S.low_light_intensity}; // Green light Background...
  // light intensity
  OutputAction LeftLightOutputH = {"PWM1", S.high_light_intensity};  // High left light intensity
  OutputAction RightLightOutputH = {"PWM5", S.high_light_intensity}; // High right light intensity

  /*********************SamplePeriod Contrast************************/
  if(S.currProtocolIndex == 6){
    if(int(random(100)) < 50){
        // 400 ms delay 
        S.DelayPeriod = 400;

        if(modality == 0){
          // spatial cue low clicks rate
          if(int(random(100)) < 50){
            PreCueContrast = int(random(1 ,3));
          }else{
            PreCueContrast = 0;
          }
          ...
          // freq cue high clicks rate
          if(int(random(100)) < 50){
            PreCueContrast = int(random(4 ,6));
          }else{
            PreCueContrast = 6;
          }
          if(int(random(100)) < 10){
      ...
      S.DelayPeriod = int(random(1 ,6)) * 400;// exponent(0.2, 0.2, 1.0); // 随机 delay period 200ms - 1000ms ; mean 200ms
      if(modality == 0){
        // spatial cue low clicks rate
        PreCueContrast = 0;
      }else if(modality == 1){
  ...
  {
  case 0: // Habitation 1: free reward train mouse to lick center port to begin a new trial
  case 1: // Habitation 2: training mouse to inhibit Early lick during the delay period 1
  {                         
    ...
    switch(S.currProtocolIndex){
      ...
      case 1:
      {
        freeRewardProb = 0; // remove free reward
    ...
    OutputAction DummyOutput;
    OutputAction RewardOutput;

    DummyOutput = {"Flag3" , 0}; // no output 
   
    S.PreCueDelayPeriod = float(random(200, 500));

    String LeftLickAction;
    String RightLickAction;
    String ActionAfterSample;
    float reward_dur = S.reward_left;
    // int PreCueInterval[7] = {58 ,45 ,35 ,26 ,20 ,15 ,11}; 16 - 64

    switch (TrialType)
    {
    case 1: // left choice ;
    {
      switch(modality){ // pre cue 
      case 0: // spatial ; low clicks rate
      {
        SampleOutput = LeftCueSoundOutput;
      }
      break;
      case 1: // freq ; // high clicks rate
      {
    ...
    case 2: // right choice ;
    {
      switch(modality){ // pre cue orientation
      case 0: // spatial ; low clicks rate
      {
        // PreCueContrast =  0; // clicks rate 16 
      ...
      case 1: // freq ; // high clicks rate
      {
        // PreCueContrast =  6; // clicks rate 64
    ...
    if (random(100) < freeRewardProb)
    { // free reward probability
      ActionAfterSample = "GiveFreeDrop";
    }
    else
    {
      ActionAfterSample = "SamplePeriod";
    }

    // global timer 
    smart.SetGlobalTimer(1,  S.PreCuePeriod + 10);

    StateTransition PreCueDelayPeriod_Cond[3] = {{"Tup", "PreCueActive"} ,{"Lick1In", "EarlyLickAborted"}, {"Lick2In", "EarlyLickAborted"}};
    ...
    StateTransition SampleCue_Cond[1]   = {{"Tup", ActionAfterSample}};

    StateTransition PreCueActive_Cond[3] = {{"Tup" ,"PreCueEventPeriod"} ,{"Lick1In", "EarlyLickAborted"}, {"Lick2In", "EarlyLickAborted"}};
    StateTransition PreCueEventPeriod_Cond[4]   = {{"Tup", "PreCueIntervalPeriod"} ,{"GlobalTimer1_End" ,"FixedDelayPeriod"} ,{"Lick1In", "EarlyLickAborted"}, {"Lick2In", "EarlyLickAborted"}};
    StateTransition PreCueIntervalPeriod_Cond[4] = {{"Tup", "PreCueEventPeriod"} ,{"GlobalTimer1_End" ,"FixedDelayPeriod"},{"Lick1In", "EarlyLickAborted"}, {"Lick2In", "EarlyLickAborted"}};

    StateTransition FixedDelayPeriod_Cond[3] = {{"Tup", "DelayPeriod"} ,{"Lick1In", "EarlyLickAborted"}, {"Lick2In", "EarlyLickAborted"}};
    StateTransition DelayPeriod_Cond[3] = {{"Tup", "SampleCue"} ,{"Lick1In", "EarlyLickAborted"}, {"Lick2In", "EarlyLickAborted"}};
    ...
    // StateTransition PreCue_Cond[3]     = {{"Tup" , "FixedDelayPeriod"},{"Lick1In", "EarlyLickAborted"}, {"Lick2In", "EarlyLickAborted"}};
    ...
    OutputAction Sample_Output[3]     	     = {SampleOutput ,GreenLightOutput ,DummyOutput};
    OutputAction EL_Output[2] = {GreenLightOutput ,DummyOutput}; // low intensity green light as a pre cue
    OutputAction FixedDelay_Output[3] = {SampleTimerCancelOutput ,GreenLightOutput ,DummyOutput};
    ...
    OutputAction PreCue_Output[3]      = {CueOutput ,GreenLightOutput ,DummyOutput};
    OutputAction FreeReward_Output[2] = {RewardOutput ,SampleOutput};
    OutputAction TrialEndCue_Output[1] = {CueOutput}; // 3000hz cue
    OutputAction PreCueDelayPeriod_Output[2]      = {GreenLightOutput ,DummyOutput};
    OutputAction SampleActive_Output[3] = {SampleTimerTrigOutput ,GreenLightOutput ,DummyOutput};
    ...
    states[10] = smart.CreateState("PreCueDelayPeriod",              S.PreCueDelayPeriod,       1 + ELInhibitaionStage[0], PreCueDelayPeriod_Cond,       0 + ELInhibitaionStage[0],           PreCueDelayPeriod_Output);

    states[12] = smart.CreateState("PreCueActive",                   10,                        1 + ELInhibitaionStage[1], PreCueActive_Cond,  1 + ELInhibitaionStage[1], SampleActive_Output);
    states[16] = smart.CreateState("PreCueEventPeriod",              5,                         2 + ELInhibitaionStage[1], PreCueEventPeriod_Cond, 1 + ELInhibitaionStage[1], PreCue_Output);
    states[17] = smart.CreateState("PreCueIntervalPeriod",           PreCueInterval[PreCueContrast],          2 + ELInhibitaionStage[1], PreCueIntervalPeriod_Cond, 0 + ELInhibitaionStage[1], EL_Output);

    // states[12] = smart.CreateState("PreCue",                         S.PreCuePeriod,            1 + ELInhibitaionStage[1], PreCue_Cond,                  1 + ELInhibitaionStage[1],           PreCue_Output);

    states[2] = smart.CreateState("FixedDelayPeriod",                random(200,400),           1 + ELInhibitaionStage[2], FixedDelayPeriod_Cond,        1 + ELInhibitaionStage[2],           FixedDelay_Output);
    states[3] = smart.CreateState("DelayPeriod",                     S.DelayPeriod,             1 + ELInhibitaionStage[3], DelayPeriod_Cond,             0 + ELInhibitaionStage[3],           EL_Output); 
    states[11] = smart.CreateState("EarlyLickAborted",               S.EarlyLickPeriod,         1,                         EarlyLickAborted_Cond,        2,                                   ErrorOutput);
    states[18] = smart.CreateState("SampleCue",                      100,                       1                        , SampleCue_Cond,               1,                                   Sample_Output); // 500ms
    states[1] = smart.CreateState("SamplePeriod",                    S.SamplePeriod,            3                        , SamplePeriod_Cond,            1,                                   Sample_Output); // 500ms

    states[4] = smart.CreateState("GiveFreeDrop",                    reward_dur,                1,                         GiveFreeDrop_Cond,            2,                                   FreeReward_Output);
    states[5] = smart.CreateState("AnswerPeriod",                    S.AnswerPeriod,            3,                         AnswerPeriod_Cond,            0,                                   Sample_Output);
    ...
    states[13] = smart.CreateState("TrialAborted",                   ELTimeOutPeriod     ,      1,                         TrialAborted_Cond,            0,                                   NoOutput); // Early lick punishment : randomly 3s ~ 8s
    states[14] = smart.CreateState("TrialEndCue",                    100,                       1,                         TrialEndCue_Cond,             1,                                   TrialEndCue_Output);
    states[15] = smart.CreateState("TrialEnd",                       random(30, 60) * 60 * 1000,2,                         TrialEnd_Cond,                0,                                   NoOutput);
  ...
  case 2: // Early training 1: use without conflict trials 2000 trials ; TimeOut and ITI increment Block 0

  case 3: // Early training 2 : Block 1
  case 4:  // retention protocol : only conflict trials Block 2

  case 5: // Test 1 : introduce the catch trials random
  case 6: // Test 2 : introduce the different delay period and pre cue period as contrast(Onset cue is CueOutput)
  {              
    ELInhibitaionStage[0] = 2;
    ELInhibitaionStage[1] = 2;
    ELInhibitaionStage[2] = 2;
    ...
    OutputAction RewardOutput;
    OutputAction PreCueOutput;
    OutputAction PreCueDelayOutput;
    ...
    case 1: // left choice ;
    {
      switch(modality){ // pre cue orientation
        case 0: // spatial ; low clicks rate
        {
          if(ContextType == 0){ // visual-only
            SampleOutput = LeftCueSoundOutput;
          }else if(ContextType == 1){ // visual-conflict
            SampleOutput = LeftHighSoundOutput;
          }else if(ContextType == 2){ // visual-coherent
        ...
        case 1: // freq ; high clicks rate 
        {
          if(ContextType == 0){ // auditory-only
            SampleOutput = LowSoundOutput;
          }else if(ContextType == 1){ // auditory-conflict
            SampleOutput = RightLowSoundOutput;
          }else if(ContextType == 2){ // auditory-coherent
            SampleOutput = LeftLowSoundOutput;
          }
        }
        break;
    ...
    // PreCueDelayOutput
    if(S.currProtocolIndex == 6){
      PreCueDelayOutput = DummyOutput;
    }else{
      PreCueDelayOutput = PreCueOutput;
    }
    ...
    states[12] = smart.CreateState("PreCueActive",                   10,                        1 + ELInhibitaionStage[1], PreCueActive_Cond,  1 + ELInhibitaionStage[1], SampleActive_Output);
    states[19] = smart.CreateState("PreCueEventPeriod",              5,                         2 + ELInhibitaionStage[1], PreCueEventPeriod_Cond, 1 + ELInhibitaionStage[1], PreCue_Output);
    states[20] = smart.CreateState("PreCueIntervalPeriod",           PreCueInterval[PreCueContrast],          2 + ELInhibitaionStage[1], PreCueIntervalPeriod_Cond, 0 + ELInhibitaionStage[1], EL_Output);

    // states[12] = smart.CreateState("PreCue",                         S.PreCuePeriod,            1 + ELInhibitaionStage[1], PreCue_Cond,                  1 + ELInhibitaionStage[1],           PreCue_Output);

    states[2] = smart.CreateState("FixedDelayPeriod",                200,                       1 + ELInhibitaionStage[2], FixedDelayPeriod_Cond,        1 + ELInhibitaionStage[2],           FixedDelay_Output);
    states[3] = smart.CreateState("DelayPeriod",                     S.DelayPeriod,             1 + ELInhibitaionStage[3], DelayPeriod_Cond,             0 + ELInhibitaionStage[3],           EL_Output); 
    states[11] = smart.CreateState("EarlyLickAborted",               100,         1 ,                        EarlyLickAborted_Cond,        2,                                   ErrorOutput);
    states[21] = smart.CreateState("SampleCue",                      100,                       1                        , SampleCue_Cond,               1,                                   Sample_Output); // 500ms
    states[1] = smart.CreateState("SamplePeriod",                    S.SamplePeriod,            3                        , SamplePeriod_Cond,            1,                                   Sample_Output); // 500ms
    ...
    states[16] = smart.CreateState("StopLicking",                    S.StopLickingPeriod,       4,                         StopLicking_Cond,             0,                                   NoOutput);
    states[17] = smart.CreateState("StopLickingReturn",              100,                       1,                         Tup_StopLicking_Cond,         0,                                   NoOutput); 
    states[8] = smart.CreateState("NoResponse",                      10                        ,1,                         NoResponse_Cond,              0,                                   NoOutput); 
    states[9] = smart.CreateState("ErrorTrial",                      500,                       1,                         ErrorTrial_Cond,              1,                                   ErrorOutput); 
    states[18]  = smart.CreateState("TimeOut",                       S.TimeOut+S.extra_TimeOut, 1,                         Tup_Exit_Cond,                0,                                   NoOutput);
   
    states[13] = smart.CreateState("TrialAborted",                   S.EarlyLickPeriod ,          1,                         TrialAborted_Cond,            0,                                   NoOutput); // Early lick punishment : randomly 3s ~ 8s
    states[14] = smart.CreateState("TrialEndCue",                    100,                       1,                         TrialEndCue_Cond,             1,                                   TrialEndCue_Output);
    states[15] = smart.CreateState("TrialEnd",                       random(30, 60) * 60 * 1000,2,                         TrialEnd_Cond,                0,                                   NoOutput);
...
void UpdateTrialOutcome()
{
  ...
  */
  ...
  for (int i = 0; i < RECORD_TRIALS - 1; i++)
  {
    ...
    S.ModalityHistory[i] = S.ModalityHistory[i + 1]; //
    S.ContextHistory[i] = S.ContextHistory[i + 1]; //
  }
  ...
  S.ModalityHistory[RECORD_TRIALS - 1] = modality;
  S.ContextHistory[RECORD_TRIALS - 1] = ContextType;
  ...
  if (S.OutcomeHistory[RECORD_TRIALS - 1] != 3 && S.OutcomeHistory[RECORD_TRIALS - 1] != 0)
  {
    ...
    if(modality == 0){ // visual
      if(ContextType == 0){
        S.currProtocolPerf[0] = S.currProtocolPerf[0] * Alpha100 + (1.0 - Alpha100) * CurrOutcome;
        S.hardestTrials[0]++;
      }else if(ContextType == 1){
        S.currProtocolPerf[1] = S.currProtocolPerf[1] * Alpha100 + (1.0 - Alpha100) * CurrOutcome;
        S.hardestTrials[1]++;
      }
    }else if(modality == 1){ // auditory
      if(ContextType == 0){
        S.currProtocolPerf[2] = S.currProtocolPerf[2] * Alpha100 + (1.0 - Alpha100) * CurrOutcome;
        S.hardestTrials[2]++;
      }else if(ContextType == 1){
        S.currProtocolPerf[3] = S.currProtocolPerf[3] * Alpha100 + (1.0 - Alpha100) * CurrOutcome;
        S.hardestTrials[3]++;
      }
    }
  }
  for(int i=0;i<4;i++){
    currProtocolPerf_corrected[i] = S.currProtocolPerf[i] / (1.0 - pow(Alpha100 ,S.hardestTrials[i] + 1));
  }
  // S.currProtocolPerf = round((float)Outcomes_sum / recent_trials * 100);

  Perf100 = 0;
  EarlyLick100 = 0;
  for (int i = RECORD_TRIALS - 100; i < RECORD_TRIALS; i++)
  {
    if (S.OutcomeHistory[i] == 1)
    {
      Perf100++;
...
void SendTrialInfo2PC()
{
  ...
  float average_perf = 0.0;
  int average_perf_int = 0;
  for(int i=0;i<4;i++){
    average_perf = average_perf + currProtocolPerf_corrected[i];
  }
  average_perf_int = int(average_perf / 4.0 * 100);

  TrialInfo_str += String(average_perf_int);
  TrialInfo_str += ",";
...
void autoChangeProtocol()
{
  switch (S.currProtocolIndex)
  {
  case 0: // habitation 1 block 0
    if ((S.currProtocolTrials > 1000 && EarlyLick100 <= 20) || (S.currProtocolTrials > 2000))
    {                        
      S.currProtocolIndex = 1; // habitation 2
      S.currProtocolTrials = 0;
      for(int i=0;i<4;i++){
        S.hardestTrials[i] = 0;
        S.currProtocolPerf[i] = 0;
      }
  ...
  case 1: // habitation 2 block 0
    if (S.currProtocolTrials > 2000)
    {
      ...
      S.EarlyLickPeriod     = 100;
      S.TrialPresentMode = 3; // 0"pattern",1"random",2"antiBias",3"fixed"
    }
    break;

  case 2: // Early training 1 block 0 only without conflict trials
    if (S.currProtocolTrials > 2000 && int(currProtocolPerf_corrected[0] * 100) > 75 && int(currProtocolPerf_corrected[2] * 100) > 75 )
    {
      S.currProtocolIndex = 3; // Early training 2
      S.currProtocolTrials = 0;
      for(int i=0;i<4;i++){
        S.hardestTrials[i] = 0;
        S.currProtocolPerf[i] = 0;
  ...
  case 3: // Early training 2 ; curriculum training block 1 (Block num == 50)
    // curriculum training
    S.TrialPresentMode = 3;
    if (S.currProtocolTrials > 100 && int(currProtocolPerf_corrected[1] * 100) > 75 && int(currProtocolPerf_corrected[3] * 100) > 75) // 其中 S.currProtocolPerf 记录的perf 为conflict trials 100 的EMA perf
    {
      if(TrialOutcome != 0 && TrialOutcome!= 3){
        S.retention_counter++;
      }
    }
    if (S.retention_counter >= 5){
      S.currProtocolIndex = 4; // retention period
      S.currProtocolTrials = 0;
      for(int i=0;i<4;i++){
        S.hardestTrials[i] = 0;
        S.currProtocolPerf[i] = 0;
  ...
  case 4: // Retention period block 2 block size is 5
    S.AnswerPeriod = 3000;
    S.EarlyLickPeriod = exponent(4.0, 3.0, 8.0);
    S.TimeOut = 4000;
    S.StopLickingPeriod = 400;
    if (S.currProtocolTrials % 100 == 0 && int(currProtocolPerf_corrected[1] * 100) > 75 && int(currProtocolPerf_corrected[3] * 100) > 75)
    {
      if(TrialOutcome != 0 && TrialOutcome!= 3){
        S.retention_counter++;
      }
    ...
    BlockSize = 12 - S.retention_counter * 2;
    
    break;

  case 5: // Test 1 block 3
    if (S.currProtocolTrials > 5000)
    {
      S.currProtocolIndex = 6;  // Test 2 block 3
      S.currProtocolTrials = 0;
      for(int i=0;i<4;i++){
        S.hardestTrials[i] = 0;
        S.currProtocolPerf[i] = 0;
    ...
    // param advanced
    switch (int(S.currProtocolTrials / 1000))
    {
    case 0:
      S.SamplePeriod = 500;
      S.TimeOut = 4000;
...
void trialSelection()
{
  ...
  {       // 0"pattern trial type and modality",1"random trial type and modaltiy",2"antiBias trial type and modality",3"fixed"
  case 0: // Pattern: 3 left 3 right : same as nature 2017 attention
    // trial type pattern
...
void TrialBlock()
{ //TODO block length ?? 6 
   // *********************************** modality and contexttype patterns************************************/
   int modality_switch_indicator = 0; 
   int context_switch_indicator = 0; 
   switch (S.currProtocolIndex)
   {
   case 0:
   case 1:
   case 2: // Block 0
    {
      ...
      for (int i = RECORD_TRIALS - 1; i >= 0; i--)
      {
        if (S.ModalityHistory[i] == S.ModalityHistory[RECORD_TRIALS - 1])
        {
          if (S.OutcomeHistory[i] == 1)
          {
            modality_switch_indicator++;
          }
        }
        else
        {
      ...
      if (modality_switch_indicator >= BlockSize)
      {
        if (S.ModalityHistory[RECORD_TRIALS - 1] == 1)
        {
          modality = 0;
        }
        else
      ...
      {
        modality = S.ModalityHistory[RECORD_TRIALS - 1];
      }
    }
    break;
  case 3: // block 50 -> 40 -> 30 -> 20 -> 10
    {
      // modality,context -> (0 ,0) ,(1 ,0) ,(1 ,1) ,(1 ,1),(0 ,1),(0 ,1) 
      for (int i = RECORD_TRIALS - 1; i >= 0; i--)
      {
        if (S.ModalityHistory[i] == S.ModalityHistory[RECORD_TRIALS - 1] && S.ContextHistory[i] == S.ContextHistory[RECORD_TRIALS - 1])
        {
          if (S.OutcomeHistory[i] == 1)
          {
            modality_switch_indicator++;
            context_switch_indicator++;
          }
        }
        else
        {
      ...
      if (modality_switch_indicator >= BlockSize && context_switch_indicator >= BlockSize)
      {
        if (S.ModalityHistory[RECORD_TRIALS - 1] == 1 && S.ContextHistory[RECORD_TRIALS - 1] == 0) // without conflict and auditory cue
        {
          modality = 1;
          ContextType = 1;
        }
        else if(S.ModalityHistory[RECORD_TRIALS - 1] == 1 && S.ContextHistory[RECORD_TRIALS - 1] == 1 && context_switch_indicator >= 2 * BlockSize) // with conflict and auditory cue
        {
          modality = 0;
          ContextType = 1;
        }
        else if(S.ModalityHistory[RECORD_TRIALS - 1] == 0 && S.ContextHistory[RECORD_TRIALS - 1] == 0) // without conflict and vision cue
        {
          modality = 1;
          ContextType = 0;
        }
        else if(S.ModalityHistory[RECORD_TRIALS - 1] == 0 && S.ContextHistory[RECORD_TRIALS - 1] == 1 && context_switch_indicator >= 2 * BlockSize) // with conflict and vision cue
        {
          modality = 0;
          ContextType = 0;
        }
      ...
      {
        modality = S.ModalityHistory[RECORD_TRIALS - 1];
        ContextType = S.ContextHistory[RECORD_TRIALS - 1];
      }
    }
    break;
  case 4: // block 2
    ...
    break;
  case 5:
  case 6: // block 3(random)
    {
      ContextType = 1;
      modality = int(random(2)); // 0 or 1
    }
    break;
   default:
    break;
...
int write_SD_para_S()
{
  ...
  if (dataFile)
  {
    ...
    for (int i = 0; i < 4; i++)
    {
      dataFile.print(S.currProtocolPerf[i] ,4);
      dataFile.print("; ");
    }
    dataFile.println();
    // TrialPresentMode
    ...
    // ModalityHistory
    dataFile.print("ModalityHistory = ");
    for (int i = 0; i < RECORD_TRIALS; i++)
    {
      dataFile.print(S.ModalityHistory[i]);
      dataFile.print("; ");
    }
    dataFile.println();

    // ContextHistory
    dataFile.print("ContextHistory = ");
    for (int i = 0; i < RECORD_TRIALS; i++)
    {
      dataFile.print(S.ContextHistory[i]);
      dataFile.print("; ");
    }
    dataFile.println();
    ...
    for (int i = 0; i < 4; i++)
    {
      dataFile.print(S.hardestTrials[i]);
      dataFile.print("; ");
    }
    dataFile.println();
...
int read_SD_para_S()
{
  ...
  if (dataFile)
  {
    ...
    {
      string_tmp = dataFile.readStringUntil(';');
     S.currProtocolPerf[i] = string_tmp.toFloat();
    }
    string_tmp = dataFile.readStringUntil('\n');
    // TrialPresentMode
    string_tmp = dataFile.readStringUntil('=');
    ...
    {
      string_tmp = dataFile.readStringUntil(';');
      S.ModalityHistory[i] = string_tmp.toInt();
    }
    string_tmp = dataFile.readStringUntil('\n');
    ...
    {
      string_tmp = dataFile.readStringUntil(';');
      S.ContextHistory[i] = string_tmp.toInt();
    }
    string_tmp = dataFile.readStringUntil('\n');
    ...
    {
      string_tmp = dataFile.readStringUntil(';');
      S.hardestTrials[i] = string_tmp.toInt();
    }
    string_tmp = dataFile.readStringUntil('\n');
  }
  else
...
int write_SD_trial_info()
{
  ...
  if (dataFile)
  {
    ...
    dataFile.print(ContextType); 
    dataFile.print(" ");
    // PreCueContrast
    dataFile.print(PreCueContrast); 
    dataFile.print(" ");
