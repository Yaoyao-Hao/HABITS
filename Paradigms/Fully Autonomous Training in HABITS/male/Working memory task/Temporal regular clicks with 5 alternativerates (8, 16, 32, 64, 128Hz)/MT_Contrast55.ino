...
{ // 使用 trial by trial 的更新方式 ；其中超参数有： 1. Fast EMA alpha ; 2. Slow EMA alpha ; 3. ALP EMA alpha 
  float FastEMA[4] = {0}; // EMA 500
  float SlowEMA[4] = {0}; // EMA 1000
  float ALP[4] = {0};

  int CurrParamsIndex = 0; // 0 or 1
  int subParamsNum = 4; 
  int miniBatch    = 1; // 每次选择得到的一个参数都会连续训练 n 个trials ： trialBlock
  ...
  float NormSoftMax_T = 0.05; 

  unsigned int t[4] = {1 ,1 ,1 ,1};                  // update step
} CurriculumParams;
CurriculumParams CP;

int CurriculumSpace[4] = {1 ,2 ,3 ,4}; ; // ContextType 
...
void curriculumChangeProtocol() // WSLS antibias 
{
  float currReward;
  ...
  if(CP.currBatchIndex >= CP.miniBatch){
    if(S.OutcomeHistory[RECORD_TRIALS - 1] == 1){
        ...
        CP.CurrParamsIndex = normSoftMaxMin(CP.subParamsNum ,CP.ALP ,CP.NormSoftMax_T ,0 ,0); // softmax without normlization 0 ,1 ,2 ,3
        }
        // get the stimulus from SGM
        TrialTypeContrast[0] = random(5);
        TrialTypeContrast[1] = random(5);
        int difff;
        difff = TrialTypeContrast[0] - TrialTypeContrast[1];
        while(abs(difff) != CurriculumSpace[CP.CurrParamsIndex]){
          TrialTypeContrast[0] = random(5);
          TrialTypeContrast[1] = random(5);
          difff = TrialTypeContrast[0] - TrialTypeContrast[1];
        }
