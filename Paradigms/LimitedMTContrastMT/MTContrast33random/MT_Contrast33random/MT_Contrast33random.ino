...
byte TrialTypeContrast[2] = {0 ,2}; // X1 & X2 ; range from 0 ~ 4
...
void construct_matrix_and_Run()
{
  ...
  {                             
    ...
    StateTransition DelayPeriod_Cond[3] = {{"Tup" ,"TestActive"},{"Lick1In", "EarlyLickDelay"}, {"Lick2In", "EarlyLickDelay"}};
    ...
    states[21] = smart.CreateState("EarlyLickDelay",    S.EarlyLickPeriod,    1, EarlyLickDelay_Cond,  0 + ELInhibitaionStage[2], EL_Output);
  ...
  {                  
    ...
    case 2:
      ...
      break;
    case 5: // Test 1 probe trials
      {
        S.DelayPeriod = 500;
        if(int(random(100)) < 10){ // probe trials
        modality = 1;
        // {0 ,0} or {4 ,4}
        if(int(random(2)) == 0){
          TrialTypeContrastPC[0] = 0;
          TrialTypeContrastPC[1] = 0;
        }else{
          TrialTypeContrastPC[0] = 4;
          TrialTypeContrastPC[1] = 4;
        }
        Sample_EventsInterval = S.EventsInterval[TrialTypeContrastPC[0]];
        Test_EventsInterval = S.EventsInterval[TrialTypeContrastPC[1]];
      ...
      break;
    case 6: // Test 2 probe trials advanced Delay period
    {
        // TODO Curriculum(); 
       if(int(random(100)) < 10){ // probe trials
        modality = 1;
    ...
    states[24] = smart.CreateState("EarlyLickDelay",    S.EarlyLickPeriod,    1, EarlyLickDelay_Cond,  0 + ELInhibitaionStage[2], EL_Output);
...
void autoChangeProtocol()
{
  ...
  case 3: // Early training
    ...
    S.EarlyLickPeriod     = 10;
    S.DelayPeriod =250;
    if (S.currProtocolTrials > 100 && int(Perf100_corrected * 100) > 85 && int(TrialContrastRate_corrected * 100) > 85 ) // TrialContrastRate_corrected 记录最难contrast 的performance
    {
  ...
  case 4: // retention 
    if (S.currProtocolTrials  % 100 == 0 && int(Perf100_corrected * 100) > 85 && int(TrialContrastRate_corrected * 100) > 85)
    {
      S.retention_counter++;
    }
...
void trialSelection() { // trial type: 1 left 2 right ; trialtype contrast : (X1 ,X2) X ->(0 ,2 ,4)
  ...
  switch (S.TrialPresentMode) {
    case 0: // limited + random 
    {
      ...
      TrialTypeContrast[0] = int(random(0 ,3)) * 2;
      TrialTypeContrast[1] = int(random(0 ,3)) * 2;
      int difff;
      difff = TrialTypeContrast[0] - TrialTypeContrast[1];
      while(abs(difff) != 2){
        TrialTypeContrast[0] = int(random(0 ,3)) * 2;
...
void SelectTrialType(struct DATAModel *data ,byte Kernel ,byte limitedSelect) { 
    ...
    if(limitedSelect){
      ...
      min[0] = ArgMinValue[0][2]; // 8 Hz VS 32 Hz
      min[1] = 0;
      min[2] = 2;
      for(int i = 0; i < 5; i++)
        for(int j = 0; j < 5; j++)
          if(i != j && i%2 == 0 && j%2 == 0 && abs(i-j) == 2) // 3*3 SGM 
