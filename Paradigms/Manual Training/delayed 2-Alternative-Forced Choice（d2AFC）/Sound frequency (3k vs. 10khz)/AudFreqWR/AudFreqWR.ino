...
unsigned long last_session_read_time = 0;
...
OutputAction LowSoundOutput   = {"tPWM2", 1}; // 3000Hz
OutputAction LowMidSoundOutput   = {"tPWM2", 3}; // 3816Hz
OutputAction LowHardSoundOutput   = {"tPWM2", 4}; // 4855Hz

OutputAction HighSoundOutput  = {"tPWM2", 2};  // 10000 Hz
OutputAction HighMidSoundOutput  = {"tPWM2", 5}; // 7860 Hz
OutputAction HighHardSoundOutput  = {"tPWM2", 6}; // 6177 Hz

OutputAction LowSoundOutputArray[3] = {LowSoundOutput ,LowMidSoundOutput ,LowHardSoundOutput};  
OutputAction HighSoundOutputArray[3] = {HighSoundOutput ,HighMidSoundOutput ,HighHardSoundOutput};  
...
OutputAction RewardFlagOutput = {"Flag2" ,1};
...
int RewardCounter = 0;
int RewardCounterFlag = 1;
int WaterRestriction = 400;

int PriorBlockCounter = 0;
int CurrentBlockSize = 50; // exponential 
bool modality = true; // 0 20-80 ;1 80-20
float BlockLeftTrialProb = 0.8;
int Contrast = 0; // 0 -> 2 easy -> hard
...
void setup() {
  ...
  smart.setTruePWMFrequency(2, 4, 4855, 128);  // (go cue) byte tPWM_num, byte freq_num, uint32 frequency, byte duty
  smart.setTruePWMFrequency(2, 5, 7860, 128);  // (go cue) byte tPWM_num, byte freq_num, uint32 frequency, byte duty
  smart.setTruePWMFrequency(2, 6, 6177, 128);  // (go cue) byte tPWM_num, byte freq_num, uint32 frequency, byte duty
  FLASHLED(200);
...
void loop() {
  ...
  if (digitalRead(switchPin) == 0 && pause_signal_PC == 0) { // if yes, run the state matrix
    ...
    if(smartFlag[1] == 1 && RewardCounterFlag == 1){
      RewardCounterFlag = 0;
      RewardCounter++;
    }
    if(smartFlag[1] == 0){
    ...
    // 判断是否要结束这个session
    if(RewardCounter >= WaterRestriction){
      pause_signal_PC = 1; // puase state machine
    }
    ...
    if (paused == 0) {
      ...
      udpPrint("M: Program PAUSED!!!");
        ...
        if(S.currProtocolIndex != 9){
          if(RewardCounter < WaterRestriction){ // 0.25ml 100 correct trials ; 400 correct trials  -> 1ml water consumed
            for(int i=0;i<WaterRestriction - RewardCounter;i+=2){
    ...
    // if (millis() - last_weight_read_time > 3600 * 24) 
    // {
    //   pause_signal_PC = 0;
    //   last_session_read_time = millis();
    // }
    // Flashing LED
    if (ledState == LOW) {
      ledState = HIGH;
...
OutputAction RightLightOutputH = {"PWM5", S.high_light_intensity}; // High right light intensity
  ...
  switch (S.currProtocolIndex) {
    case 0: { // PROTOCOL 0: teaching animal to lick both side
        ...
        OutputAction Reward_Output[2]          = {RewardOutput ,RewardFlagOutput};
        ...
        states[4]  = smart.CreateState("GiveFreeDrop",      reward_dur,           1, GiveFreeDrop_Cond,    2, Reward_Output);
        states[5]  = smart.CreateState("AnswerPeriod",      S.AnswerPeriod,       3, AnswerPeriod_Cond,    0, NoOutput);
        states[6]  = smart.CreateState("Reward",            reward_dur,           1, Reward_Cond,          2, Reward_Output);
	...
	// trial block prior  PC {3000 ; 3816 ;4855 ;6177 ;7860 ;10000}
	case 8: // water Restriction
  case 9: // without Water Restriction
	{
		    OutputAction SampleOutput;
        OutputAction RewardOutput;
		    unsigned int DelayPeriod_trial;
        ...
        Contrast = int(random(3));
        switch (TrialType) {
          case 1: // left-low sound
            SampleOutput    = LowSoundOutputArray[Contrast];  
            ...
            break;
          case 2: // right-high sound
            SampleOutput    = HighSoundOutputArray[Contrast];  
...
void autoChangeProtocol() {
  switch (S.currProtocolIndex) {
    ...
    case 7:
	  ...
	  S.retention_counter = 0;
	  S.currProtocolIndex = 8; // advanced to Protocol prior and PC
        S.currProtocolTrials = 0;
        S.currProtocolPerf = 0;
    ...
    case 8:
	  if(S.currProtocolTrials > 5000){
        S.retention_counter = 0;
        S.currProtocolIndex = 9; // without water restriction
        WaterRestriction = 100000;
        S.currProtocolTrials = 0;
        S.currProtocolPerf = 0;
...
void trialSelection() {
  ...
  switch (S.TrialPresentMode) { // 0"pattern",1"random",2"antiBias",3"fixed"
    ...
    case 3: { // TrialPresentMode     'fixed'
      ...
      break;
    case 4: { // "trial block" 20 - 100 trials 50 mean 
        if (S.OutcomeHistory[RECORD_TRIALS - 1] != 0 && S.OutcomeHistory[RECORD_TRIALS - 1] != 3) {
          PriorBlockCounter++;
        } 
        // Serial.print(PriorBlockCounter);
        // Serial.print(" ");
        // CurrentBlockSize = exponent(50.0 ,20.0 ,100.0);
        // Serial.print(CurrentBlockSize);
        // Serial.print(" ");
        // Serial.print(modality);
        // Serial.print(" ");

        if (PriorBlockCounter >= CurrentBlockSize) {
          CurrentBlockSize = exponent(50.0 ,20.0 ,100.0);
          PriorBlockCounter = 0;
          modality = !modality;
          if(modality){
            BlockLeftTrialProb = 0.8;
          }else{
            BlockLeftTrialProb = 0.2;
          }
        } 

        if (random(100) <= (BlockLeftTrialProb * 100)) {
            TrialType = 1;
          } else {
            TrialType = 2;
          }
...
int write_SD_para_S() {
  ...
  if (dataFile) {
    ...
    // Session
    dataFile.print("Session = ");
    dataFile.println(S.Session);
	// low_light_intensity
...
int write_SD_trial_info() {
  ...
  if (dataFile) {
    ...
    dataFile.print(10000); // 分隔符
    dataFile.print(" ");

    dataFile.print(modality);
    dataFile.print(" ");

    dataFile.print(Contrast);
    dataFile.print(" ");

    dataFile.print(S.Session);
    dataFile.print(" ");
