...
/********** Trial related **********/
#define RECORD_TRIALS 1000      // record recent 100 trials history
...
typedef struct
{
  ...
  byte ChoiceHistory[RECORD_TRIALS] = {}; // 0 no-response; 1 left; 2 right; 3 others;
...
byte Choice    =  3; // 1 left ; 2 right ; 3 others
...
int modality = 0; // 0: L:R ; 1 R:L
int ContextType = 0; // 0  6:1 ; 1 : 52.5:17.5
...
void construct_matrix_and_Run()
{
  ...
  {
  case 0: // Habitation 1: free reward train mouse to lick center port to begin a new trial ; answer period : 2s 
  case 1: // Habitation 2: training mouse to inhibit Early lick during the delay period  ;alternately L/R Trial Type ; DelayPeriod 200ms -> 1000ms ;per 200 choiced trials advanced; 20% EL rate and 75% correct rate 500 trials averaged
  case 2: // Habitation 3: alternately L/R choice ; delayperiod 1000ms ~ 1500ms ,500ms inteval ; randomly ; 75% correct rate 500 trials averaged

  case 3: // Full task : delayperiod : 1s - 1.5s ,500ms inteval;   dynamic farge task block size 250-300 P(high) > 65% or 57.5% ; 600 trials ,respectively
  case 4:  // block size 200-250
  case 5:  // block size 150-200
  case 6:  // block size 100-150
  case 7:  // block size 50-100

  case 8: // test
  {              
    int freeRewardProb = 0;
    S.DelayPeriod = random(2 ,4) * 500; // 1000 , 1500
    S.EarlyLickPeriod = 300;
    S.SamplePeriod = exponent(1 ,0.5 ,2); // 0.5 - 2.5s ; mean 1s
    ELInhibitaionStage[0] = 2;

    switch(S.currProtocolIndex){
      case 0:
      { 
        ELInhibitaionStage[0] = 0;
        S.DelayPeriod = 200;
        S.EarlyLickPeriod = 100;
        S.SamplePeriod = exponent(1 ,0.5 ,2);
        freeRewardProb = 1;
      }
      break;
      case 1:
      {
        int steps = int(S.currProtocolTrials / 500) + 1;
        if(steps > 5){
          steps = 5;
        }
        S.DelayPeriod = steps * 200;
        S.SamplePeriod = exponent(1 ,0.5 ,2);
        S.EarlyLickPeriod = 50 * steps + 50;
      }
      break;
      default:
        break;
    ...
    {
      LeftLickAction = "Reward";
      RightLickAction = "Error"; // no error trial
    ...
    case 2: // right choice ;
    {
      LeftLickAction = "Error"; // no error trial
      RightLickAction = "Reward";
    ...
    StateTransition PreCueDelayPeriod_Cond[3] = {{"Tup", "ResponseCue"} ,{"Lick1In", "EarlyLickAborted"}, {"Lick2In", "EarlyLickAborted"}};
    StateTransition TrialStart_Cond[1] = {{"Tup", "PreCueDelayPeriod"}}; 
    StateTransition ResponseCue_Cond[1] = {{"Tup", "AnswerPeriod"}};
    StateTransition AnswerPeriod_Cond[3] = {{"Lick1In", "LeftLick"}, {"Lick2In", "RightLick"}, {"Tup", "NoResponse"}};
    StateTransition Reward_Cond[1] = {{"Tup", "RewardConsumption"}};
    StateTransition NoResponse_Cond[2] = {{"Tup", "exit"} ,{"Lick3In", "exit"}};
    StateTransition Tup_StopLicking_Cond[1] = {{"Tup", "StopLicking"}};
    StateTransition StopLicking_Cond[3] = {{"Lick1In", "StopLickingReturn"}, {"Lick2In", "StopLickingReturn"} , {"Tup", "TrialEnd"}}; 
    StateTransition EarlyLickAborted_Cond[1]     = {{"Tup" ,"PreCueDelayPeriod"}};
    StateTransition TrialAborted_Cond[1]   = {{"Tup", "TrialEnd"}};
    StateTransition TrialEnd_Cond[1] = {{"Tup", "exit"}};
    StateTransition LeftLick_Cond[1]   = {{"Tup", LeftLickAction}};
    StateTransition RightLick_Cond[1]   = {{"Tup", RightLickAction}};
    StateTransition Error_Cond[1]   = {{"Tup", "StopLicking"}};
   
    OutputAction LeftLick_Output[2]     	     = {LowSoundOutput ,LeftWaterOutput};
    OutputAction RightLick_Output[2]     	     = {HighSoundOutput ,RightWaterOutput};
    ...
    OutputAction PreCueDelayPeriod_Output[3]      = {LeftLightOutput ,RightLightOutput ,GreenLightOutputBackground};


    gpSMART_State states[15] = {};
    // visual flash states and conditions
    states[0] = smart.CreateState("TrialStart",                      500,                       1,                         TrialStart_Cond,              3,                                   PreCueDelayPeriod_Output); // msec
    states[1] = smart.CreateState("PreCueDelayPeriod",               S.DelayPeriod,       1 + ELInhibitaionStage[0],       PreCueDelayPeriod_Cond,       0,                                   NoOutput);
    states[2] = smart.CreateState("ResponseCue",                     100,                       1,                         ResponseCue_Cond,             1,                                   ResponseCue_Output);
    states[3] = smart.CreateState("AnswerPeriod",                    2000,                      3,                         AnswerPeriod_Cond,            1,                                   ResponseCue_Output);
    states[4] = smart.CreateState("LeftLick",                        50,                        1                        , LeftLick_Cond,                1+freeRewardProb,                    LeftLick_Output); 
    states[5] = smart.CreateState("RightLick",                       50,                        1                        , RightLick_Cond,               1+freeRewardProb,                    RightLick_Output); 
    states[8] = smart.CreateState("NoResponse",                      random(59, 60) * 60 * 1000 ,2,                         NoResponse_Cond,              0,                                   NoOutput); 
    states[9] = smart.CreateState("Error",                           10,                        1 ,                        Error_Cond,                0,                                      NoOutput);
    states[6] = smart.CreateState("Reward",                          reward_dur,                1,                         Reward_Cond,                  1,                                   Reward_Output);
    states[7] = smart.CreateState("RewardConsumption",               750,                       1,                         Tup_StopLicking_Cond,         0,                                   NoOutput);
    states[10] = smart.CreateState("StopLicking",                    200,                       3,                         StopLicking_Cond,             0,                                   NoOutput);
    states[12] = smart.CreateState("StopLickingReturn",              10,                        1,                         Tup_StopLicking_Cond,         0,                                   NoOutput); 

    states[11] = smart.CreateState("EarlyLickAborted",               S.EarlyLickPeriod,         1 ,                        EarlyLickAborted_Cond,        0,                                   NoOutput);
    states[13] = smart.CreateState("TrialAborted",                   2000 ,                     1,                         TrialAborted_Cond,            0,                                   NoOutput); // Early lick punishment : randomly 3s ~ 8s
    states[14] = smart.CreateState("TrialEnd",                       S.SamplePeriod            ,1,                         TrialEnd_Cond,                0,                                   NoOutput);
...
void UpdateTrialOutcome()
{
  ...
  */
  ...
  for (int i = 0; i < trial_res.nVisited; i++)
  {
    if (trial_res.stateVisited[i] == 4 || trial_res.stateVisited[i] == 5 || trial_res.stateVisited[i] == 8)
    { // Left || Right

      if (trial_res.stateVisited[i] == 4)
      {
        Choice = 1;
      }
      else if (trial_res.stateVisited[i] == 5)
      {
        Choice = 2;
      }else{
        Choice = 0; 
  ...
  for (int i = 0; i < RECORD_TRIALS - 1; i++)
  {
    ...
    S.ChoiceHistory[i] = S.ChoiceHistory[i + 1];
  ...
  S.ChoiceHistory[RECORD_TRIALS - 1] = Choice;
  ...
  float CurrOutcome = 0; // 1 left choifce ; 0 right choice
  // recording the last valid trial
  if (S.ChoiceHistory[RECORD_TRIALS - 1] != 3 && S.ChoiceHistory[RECORD_TRIALS - 1] != 0)
  {
    S.currProtocolTrials++;
    if (S.ChoiceHistory[RECORD_TRIALS - 1] == 1)
    {
      CurrOutcome = 1.0;
    }
    else if (S.ChoiceHistory[RECORD_TRIALS - 1] == 2)
    {
      CurrOutcome = 0.0;
    }

    if(modality == 0){ // Left high prob ; left choice prob
      if(ContextType == 0){ // 6:1
        S.currProtocolPerf[0] = S.currProtocolPerf[0] * Alpha100 + (1.0 - Alpha100) * CurrOutcome; // Choice Prob
        S.hardestTrials[0]++;
      }else if(ContextType == 1){ // 5.2:1.7
        S.currProtocolPerf[1] = S.currProtocolPerf[1] * Alpha100 + (1.0 - Alpha100) * CurrOutcome;
        S.hardestTrials[1]++;
      }
    }else if(modality == 1){ // RIght High prob ; RIght choice prob
      if(CurrOutcome == 1){
        CurrOutcome = 0;
      }else{
        CurrOutcome = 1;
...
void autoChangeProtocol()
{
  ...
  case 0: // habitation 1 
    ...
    {                        
      ...
      S.TrialPresentMode = 1; // 0"free reward",1"alternately trialtype",2"alternately choice",3"Blocked"
    }
    break;

  case 1: // habitation 2 
    S.TrialPresentMode = 1;
    // if ((S.currProtocolTrials > 3000 && EarlyLick100 <= 30 && Perf100 > 70))
    if (S.currProtocolTrials > 2)
    {
  ...
  case 2: // not used
  S.TrialPresentMode = 2;
    // if (S.currProtocolTrials > 500 && Perf100 > 70)
    if (S.currProtocolTrials > 2)
    {
      S.currProtocolIndex = 3; 
      S.currProtocolTrials = 0;
 ...
 ////////////////////////
  case 3: 
    BlockSize = 500;
    S.TrialPresentMode = 3;
    if (S.currProtocolTrials > 2000 && int(currProtocolPerf_corrected[0] * 100) > 65 && int(currProtocolPerf_corrected[2] * 100) > 65 && int(currProtocolPerf_corrected[1] * 100) > 57 && int(currProtocolPerf_corrected[3] * 100) > 57)
    {
...
void trialSelection()
{
  ...
  {       
    ...
    case 2: // alternately choice
    { 
      if(S.ChoiceHistory[RECORD_TRIALS - 1] == 1){ // left choice
        TrialType = 2; // right trial type
      }else{
        TrialType = 1; // left trial type
      }
    }
      break;
...
void TrialBlock()
{  // Trial Type ; Context Type ; Modality
   ...
   recentNum = float(BlockSize);
    ...
    for (int i = RECORD_TRIALS - 2; i >= 0; i--)
    {
      ...
      {
        if((S.ChoiceHistory[i] == 1) || (S.ChoiceHistory[i] == 2)){
          if(modality_switch_indicator < recentNum){
            Choice_temp += (S.ChoiceHistory[i] - 1); // Right prob
          }
    ...
    // hint context type
    if(modality_switch_indicator > BlockSize){ // recent blocksize trials
      // check the P(high
      pHigh = float(Choice_temp) / recentNum;
      ...
      // do the switch get the contexttype & modality : (ContextType ,modality): (0, 0) -> (0, 1) -> (1 ,0) -> (1 ,1)
      if(switch_flag == 1){
        if(ContextType == 0){
          if(modality == 0){          // 0 ,0
            ContextType = 0;
    ...
    // Get the Trial Type with WSLS antibias
    int random_value = 0;

    if((S.TrialTypeHistory[RECORD_TRIALS - 1] != 3) && (S.OutcomeHistory[RECORD_TRIALS - 1] != 1)){
      // WSLS antibias
      TrialType = S.TrialTypeHistory[RECORD_TRIALS - 1];
    }else{
      random_value = int(random(1000));
      if(random_value >= 700){
        TrialType = 3;
      }else{
        // 600 : 100 or 100 : 600
        if(random_value < (600 - ContextType * 75)){  // high choice
          TrialType = 1 + modality;
        }else{ // low choice
          TrialType = 2 - modality;
        }
      }
    }

  Serial.println(ContextType);
...
int write_SD_para_S()
{
  ...
  if (dataFile)
  {
    ...
    dataFile.println();
    // ChoiceHistory
    dataFile.print("ChoiceHistory = ");
    for (int i = 0; i < RECORD_TRIALS; i++)
    {
      dataFile.print(S.ChoiceHistory[i]);
      dataFile.print("; ");
    }
    dataFile.println();
    // OutcomeHistory
...
int read_SD_para_S()
{
  ...
  if (dataFile)
  {
    ...
    {
      string_tmp = dataFile.readStringUntil(';');
      S.ChoiceHistory[i] = string_tmp.toInt();
    }
    string_tmp = dataFile.readStringUntil('\n');
    // OutcomeHistory
    string_tmp = dataFile.readStringUntil('=');
