...
OutputAction CueOutputL = {"tPWM2", 5}; // bias go cue low
OutputAction CueOutputH = {"tPWM2" ,8}; // bias go cue high
// contrast stimuli
OutputAction LowSoundOutputContrast1 = {"tPWM2", 4}; // tPWM2 left choice 
OutputAction LowSoundOutputContrast2 = {"tPWM2", 5}; // tPWM2 left choice random left or middle

OutputAction MiddleSoundOutputContrast1 = {"tPWM2", 6}; // tPWM2 middle choice khz
OutputAction MiddleSoundOutputContrast2 = {"tPWM2", 7}; // tPWM2 middle choice khz

OutputAction HighSoundOutputContrast2 = {"tPWM2", 8}; // tPWM2 middle choice khz random middle or left
OutputAction HighSoundOutputContrast1 = {"tPWM2", 9}; // tPWM2 middle choice khz
...
int modality = 3; // sample stimulus : 0 is A ;1 is B
int TrialTypeContrast = 3;
...
void setup()
{
  ...
  // tPWM2:(8KHz - 32KHz ;     8000 Hz, 9514Hz,     11314Hz,      13454Hz, 16000Hz, 19028Hz,      22628Hz,      26909Hz and 32000 Hz)(其中 auditory clicks delay 时间 为sample period)
  ...
  // low sound contrast
  smart.setTruePWMFrequency(2, 4, 9514, 128);
  smart.setTruePWMFrequency(2, 5, 11314, 128); // (bias go cue low) 
  // middle sound contrast
  smart.setTruePWMFrequency(2, 6, 13454, 128);
  smart.setTruePWMFrequency(2, 7, 19028, 128);
  // high sound contrast
  smart.setTruePWMFrequency(2, 8, 22628, 128); // (bias go cue high)
  smart.setTruePWMFrequency(2, 9, 26909, 128);
      ...
      case 9:// contrast
        {
          if(TrialType == 1) // left
          {
            TrialTypeContrast = int(random(3));
            if(TrialTypeContrast == 0){ // simplest
              SampleOutput = LowSoundOutput;
            }else if(TrialTypeContrast == 1){
              // inmimuate 
              SampleOutput = LowSoundOutputContrast1;
            }else{
              SampleOutput = LowSoundOutputContrast2;
            }
          }
          else if(TrialType == 2) // right
          {
            TrialTypeContrast = int(random(3));
            if(TrialTypeContrast == 0){ // simplest
              SampleOutput = HighSoundOutput;
              ...
              // inmimuate 
              SampleOutput = HighSoundOutputContrast1;
            }else{// hardest
              SampleOutput = HighSoundOutputContrast2;
            }
          }
          else // middle
          {
            ...
            if(TrialTypeContrast == 0){ // simplest
              ...
              // inmimuate 
              SampleOutput = MiddleSoundOutputContrast1;
            }else{
              SampleOutput = MiddleSoundOutputContrast2;
            }
          }
        }
        break;
...
int write_SD_trial_info()
{
  ...
  if (dataFile)
  {
    ...
    dataFile.print(TrialTypeContrast);
    dataFile.print(" ");
